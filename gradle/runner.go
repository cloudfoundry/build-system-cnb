...
// Runner represents the behavior of running the gradle command to build an application.
type Runner struct {
...
func (r Runner) String() string {
	return fmt.Sprintf("Runner{ application: %s, logger: %s, grad: %s}", r.application, r.logger, r.grad)
}

func (r Runner) builtArtifact() (string, error) {
	target := filepath.Join(r.application.Root, "build", "libs")
...
func NewRunner(build libjavabuildpack.Build, gradle Gradle) Runner {
	return Runner{
		build.Application,
		build.Logger,
		gradle.Executable(),
